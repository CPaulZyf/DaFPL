...
class TrainerBase:
    ...
    def train(self, start_epoch, max_epoch, idx=-1,global_epoch=-1,is_fed=False,global_weight=None, fedprox=False, mu=0.5, discriminator=None):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            # self.before_epoch()
            self.run_epoch(idx, global_epoch, global_weight=global_weight,fedprox=fedprox,mu=mu, discriminator=discriminator)
            # self.after_epoch()
...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        if torch.cuda.is_available() and cfg.USE_CUDA:
            self.device = torch.device("cuda:"+cfg.OPTIM.GPU)
        else:
    ...
    def train(self,idx=-1,global_epoch=0,is_fed=False,global_weight=None, fedprox=False, mu=0.5, discriminator=None):
        super().train(self.start_epoch, self.max_epoch,idx,global_epoch,is_fed,global_weight,fedprox,mu,discriminator)
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self, idx=-1, global_epoch=-1, global_weight=None, fedprox=False, mu=0.5, discriminator=None):
        self.set_model_mode("train")
        losses = MetricMeter()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        ...
        for self.batch_idx, batch in enumerate(loader):
            data_time.update(time.time() - end)
            if fedprox:
                loss_summary = self.forward_backward(batch, global_weight=global_weight, fedprox=fedprox, mu=mu, discriminator=discriminator, idx=idx)
            else:
