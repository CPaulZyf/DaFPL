...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            '''
            ctx_vectors = cfg.ctx_vectors[0][:n_ctx]
...
class DaFPL(TrainerX):
    ...
    def forward_backward(self, batch, global_weight=None, fedprox=False, mu=0.5, discriminator=None, idx=-1):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.PROMPTFL.PREC
        if prec == "amp":
            with autocast():
            ...
            if fedprox:
                ...
                prompt_share = ctx[:self.cfg.domain_tokens]  # [Tsh, D]
                prompt_domain = ctx[self.cfg.domain_tokens:]
                z_sh = prompt_share.mean(dim=0, keepdim=True)  # [1, D]
                z_dm = prompt_domain.mean(dim=0, keepdim=True)
                logits_sh = discriminator(z_sh)  # [1, K]
                logits_dm = discriminator(z_dm)

                y = torch.tensor([self.dm.dataset.clientid2domain[idx]], device=logits_dm.device, dtype=torch.long)
                loss_dom = F.cross_entropy(logits_dm, y)

                log_p = F.log_softmax(logits_sh, dim=-1)  # [1, K]
                U = torch.full_like(log_p.exp(), 1.0 / self.cfg.num_domain)  # [1, K]
                loss_unif = F.kl_div(log_p, U, reduction="batchmean")

                P_local = F.normalize(prompt_share, dim=1)
                P_global = F.normalize(global_weight, dim=1)
                loss_align = (P_local - P_global).pow(2).sum()


                loss = (loss + self.cfg.lambda_dom * loss_dom + self.cfg.lambda_unify * loss_unif  + self.cfg.lambda_unify * loss_align)
